...
public final class SingleKeyModuloDatabaseShardingAlgorithm implements SingleKeyDatabaseShardingAlgorithm<Long> {
    ...
    public String doEqualSharding(final Collection<String> availableTargetNames, final ShardingValue<Long> shardingValue) {
        ...
        if (shardingValue.getValue() instanceof Long) {
            ...
            Number shard = (Number)shardingValue.getValue();
            Integer shardValue = (Integer) shard;

            for (String each : availableTargetNames) {
                if (each.endsWith(shardValue % availableTargetNames.size() + "")) {
                    return each;
                }
            }
        }
        ...
        for (Number value : values) {
            ...
            if (value instanceof Long) {
                for (String dataSourceName : availableTargetNames) {
                    if (dataSourceName.endsWith((Long)value %  availableTargetNames.size() + "")) {
                        result.add(dataSourceName);
                    }
                }
            } else {
                for (String dataSourceName : availableTargetNames) {
                    if (dataSourceName.endsWith((Integer)value %  availableTargetNames.size() + "")) {
                        result.add(dataSourceName);
                    }
                }
            }
