...
import com.gws.rdb.sharding.jdbc.unsupported.AbstractUnsupportedOperationConnection;
...
public abstract class AbstractConnectionAdapter extends AbstractUnsupportedOperationConnection {
    ...
    public final void commit() throws SQLException {
        for (Connection each : getConnections()) {
        	boolean autoCommit =each.getAutoCommit();
        	if(autoCommit){
        		each.setAutoCommit(false);
        	}
            each.commit();
        }
    }
    ...
    public final void setReadOnly(final boolean readOnly) throws SQLException {
        //源代码　this.readOnly = true;
    	this.readOnly = false;
        if (getConnections().isEmpty()) {
            recordMethodInvocation(Connection.class, "setReadOnly", new Class[] {boolean.class}, new Object[] { this.readOnly});
            return;
        }
        for (Connection each : getConnections()) {
        	each.setReadOnly(this.readOnly);
        }
    }
